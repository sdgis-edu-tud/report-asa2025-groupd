...
The S-MCDA utilizes various criteria to answer the research questions.

For each theme, the criteria are weighed based on their relative importance for the research question utilizing the Saaty matrix (also known as the analytic hierarchy process). The normalized values of all criteria in a given theme are aggregate and then combined to get concluding maps which identify key areas for potential design intervention. Maps showing the untreated data which was used for the conducted analysis can be found in the appendix.
...
To help answer this question, the analysis first must answer the following sub-questions:
...
The map shows that there the largest diversity of natural habitats are right at the edges of the city borders.
...
Combining these two layers using the criteria weights in the S-MCDA, two areas stand out due to the combination of public greenery and the diversity of amenities. 
...
![Urban life conditions along the Sstream](http://rstudio-server-edu.bk.tudelft.nl:8787/files/report-asa2025-groupd/Inserted_images/Quality%20of%20Life/URBAN%20LIFE%20CONDITIONS.png)
