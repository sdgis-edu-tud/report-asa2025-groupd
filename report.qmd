...
##########################################################
# Display distance to cluster centers
##########################################################


# Ensure cluster is a factor
grids$cluster <- as.factor(kmeans_result$cluster)

# Select the top 5 closest tiles to center per cluster
top_tiles <- grids |>
  group_by(cluster) |>
  slice_min(order_by = cluster_dist, n = 5, with_ties = FALSE) |>
  ungroup()

# Plot only those top tiles
ggplot(top_tiles) +
  geom_sf(aes(fill = cluster), color = "white", size = 0.2) +
  scale_fill_manual(values = cluster_colors) +
  theme_void() +  # removes lat/lon grid, axes, and background
  labs(
    title = "Top 5 Closest Tiles per Cluster Center",
    fill = "Cluster"
  ) +
  theme(
